package cars 
{
	...
	public class TrolleyBus extends tdCarBody
	{
				
		private var trolleySkin:BusTrolley = new BusTrolley();
		private var trolleySkinEngine:qb2FlashSpriteActor = new qb2FlashSpriteActor();
		...
		public function init(stageParam:Stage)
		{
			...
			var carWidth:Number = trolleySkin.width;
			var carHeight:Number = trolleySkin.height;
						
			
			// main car body
			
			var rectangle:qb2Body = qb2Stock.newRectBody(new amPoint2d(0, 0), trolleySkin.width, trolleySkin.height,10);
			
			trolleySkinEngine.addChild(trolleySkin);
			rectangle.actor = trolleySkinEngine;
			...
			var circle:qb2Body = qb2Stock.newCircleBody(new amPoint2d(0, 50), 25, 80);
		...
		private function addTires()
		{
			...
			var tire3:tdTire = new tdTire(new amPoint2d( leftSideX,  backY), tireWidth, tireRadius, true ,  true, false  , tireFriction, tireRollingFriction);
			tire3.actor = tireSkinEngine3;
			tire3.actor.setPosition(new amPoint2d( leftSideX, backY));
			
			var tire4:tdTire = new tdTire(new amPoint2d( rightSideX,  backY), tireWidth, tireRadius, true , true, false  , tireFriction, tireRollingFriction);
